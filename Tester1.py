...
def ai_move():
    ...
    # Check potential winning moves for the player
    player_potential_moves = check_potential_moves(1, player_score)

    # Check potential winning moves for the AI
    ai_potential_moves = check_potential_moves(2, ai_score)

    if player_potential_moves:
        # Block player's potential winning move
        x, y = player_potential_moves[0]
        grid[y][x] = 2
        ai_score, _ = count_longest_line(2, x, y, ai_score)
    elif ai_potential_moves:
        # Make an AI potential winning move
        x, y = ai_potential_moves[0]
        grid[y][x] = 2
        ai_score, _ = count_longest_line(2, x, y, ai_score)
    else:
        # No potential winning moves, make a random move
        for y in range(GRID_SIZE):
            for x in range(GRID_SIZE):
                if grid[y][x] == 0 and count == 0:
                    grid[y][x] = 2
...
def check_potential_moves(player, score_count):
    potential_moves = []
    for y in range(GRID_SIZE):
        for x in range(GRID_SIZE):
            if grid[y][x] == 0:
                ...
                grid[y][x] = 0  # Restore the original grid state

                if line_length == WIN_LENGTH - 1:
